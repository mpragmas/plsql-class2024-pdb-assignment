...
SQL> CREATE PLUGGABLE DATABASE pdb1
  2     ADMIN USER pdbadmin IDENTIFIED BY pragmas;

Pluggable database created.

-- Create another pluggable database named plsql_class2024db with admin user
SQL> create pluggable database plsql_class2024db admin user pdbadmin identified by pragmas;
...
SQL> CREATE USER pr_plsqlauca IDENTIFIED BY pragmas;

User created.

-- Grant all privileges to the newly created user
SQL> GRANT all privileges to pr_plsqlauca;
...
-- Create a new pluggable database named pr_to_delete_pdb with admin user
SQL> CREATE PLUGGABLE DATABASE pr_to_delete_pdb
  2     ADMIN USER pdbadmin IDENTIFIED BY pragmas;

Pluggable database created. -- The pluggable database is created

-- Drop the pluggable database pr_to_delete_pdb including its datafiles
SQL> DROP PLUGGABLE DATABASE pr_to_delete_pdb INCLUDING DATAFILES;
